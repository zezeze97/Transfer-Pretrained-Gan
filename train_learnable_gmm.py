...
from gan_training.train import Learnable_GMM_Trainer, update_average
from gan_training.logger import Logger
from gan_training.checkpoints import CheckpointIO
from gan_training.inputs import get_dataset
from gan_training.distributions import get_ydist
from gan_training.eval import Learnable_GMM_Evaluator
from gan_training.config import (
    load_config, build_models, build_optimizers, build_lr_scheduler, build_gmm_layer_optimizers
)
...
# Create models
gmm_components_weights = np.load(config['z_dist']['gmm_components_weight'])
n_components = gmm_components_weights.size
gmm_layer = GMM_Layer(z_dim=config['z_dist']['dim'], n_components=n_components, gmm_components_weights=gmm_components_weights, device=device)
...
gmm_optimizer = build_gmm_layer_optimizers(gmm_layer, config)

# Use multiple GPUs if possible
gmm_layer = nn.DataParallel(gmm_layer)
...
# load gmm_layer parameters
gmm_mean = torch.FloatTensor(np.load(config['z_dist']['gmm_mean']))
gmm_cov = torch.FloatTensor(np.load(config['z_dist']['gmm_cov']))
gmm_layer_state_dict = gmm_layer.state_dict()
new_dict = {}
for k,v in gmm_layer.named_parameters():
    # get current gmm component number
    current_components_index = int(k.split('.')[-2])
    if k.split('.')[-1] == 'bias':
        print('loading num ',current_components_index,' gmm bias...')
        new_dict[k] = gmm_mean[current_components_index,:]
    if k.split('.')[-1] == 'weight':
        print('loading num ',current_components_index,' gmm cov...')
        new_dict[k] = torch.linalg.cholesky(gmm_cov[current_components_index,:])
gmm_layer_state_dict.update(new_dict)
gmm_layer.load_state_dict(gmm_layer_state_dict)
print('pretrained gmm layer loaded!')
...
# Evaluator
evaluator = Learnable_GMM_Evaluator(generator_test, gmm_layer, ydist,
...
gmm_scheduler = build_lr_scheduler(gmm_optimizer, config, last_epoch=it)

# Trainer
trainer = Learnable_GMM_Trainer(
    gmm_layer, generator, discriminator, g_optimizer, d_optimizer,gmm_optimizer,
...
while flag:
    ...
    for x_real, y in train_loader:
        ...
        gmm_lr = gmm_optimizer.param_groups[0]['lr']
        logger.add('learning_rates', 'discriminator', d_lr, it=it)
        logger.add('learning_rates', 'generator', g_lr, it=it)
        logger.add('learning_rates', 'gmm_layer', gmm_lr, it=it)

        x_real, y = x_real.to(device), y.to(device)
        y.clamp_(None, nlabels-1)

        # Discriminator updates
        dloss, reg = trainer.discriminator_trainstep(x_real, y)
        d_scheduler.step()
        ...
        # Generators updates
        if ((it + 1) % d_steps) == 0:
            if epoch_idx > config['training']['gmm_layer_trainable_epoch']:
                gloss = trainer.generator_trainstep(y, frozen_gmm_layer=False)
            else:
                gloss = trainer.generator_trainstep(y, frozen_gmm_layer=True)
