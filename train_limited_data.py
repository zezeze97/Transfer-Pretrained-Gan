...
from gan_training.train import Trainer_Limited_Data, update_average
...
z_aux_dist =  get_zdist(dist_name='gauss',dim=config['z_dist']['dim'], device=device)
...
# Aux generator
aux_generator = copy.deepcopy(generator)
...
# Trainer
trainer = Trainer_Limited_Data(
    generator, aux_generator, discriminator, g_optimizer, d_optimizer,
    ...
    mmd_loss_lambda=config['training']['mmd_loss_lambda'],
    aux_real_lambda=config['training']['aux_real_lambda']
)
...
while flag:
    ...
    for x_real, y in train_loader:
        ...
        z_aux=z_aux_dist.sample((batch_size,))
        dloss, reg = trainer.discriminator_trainstep(x_real, y, z, z_aux)
        d_scheduler.step()
        ...
        if ((it + 1) % d_steps) == 0:
            if config['z_dist']['type'] == 'gmm2gauss':
                ...
                z = zdist.sample((batch_size,))
            gloss, g_classification_loss, g_mmd_loss = trainer.generator_trainstep(x_real, y, z)
            logger.add('losses', 'generator_loss', gloss, it=it)
            logger.add('losses', 'generator_cls_loss', g_classification_loss, it=it)
            logger.add('losses', 'generator_mmd_loss', g_mmd_loss, it=it)
