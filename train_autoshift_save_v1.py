...
from gan_training.train import Trainer, update_average, Trainer_autoshift
from gan_training.logger import Logger
from gan_training.checkpoints import CheckpointIO
from gan_training.inputs import get_dataset
from gan_training.distributions import get_ydist, get_zdist
from gan_training.eval import Evaluator, Evaluator_autoshift, Evaluator_autoshift_save
from gan_training.config import (
    load_config, build_models, build_optimizers, build_lr_scheduler, build_optimizers_autoshift,
)
import numpy as np
import math
from collections import OrderedDict

from gan_training.models.vanilla_vae import VanillaVAE
...
autoshift = VanillaVAE(in_channels=3, latent_dim=config['z_dist']['dim'])
print(generator)
print(discriminator)
print(autoshift)
...
autoshift = autoshift.to(device)

# g_optimizer, d_optimizer = build_optimizers(
#     generator, discriminator, config
# )
g_optimizer, d_optimizer, s_optimizer = build_optimizers_autoshift(
    generator, discriminator, autoshift, config
)

# Use multiple GPUs if possible
generator = nn.DataParallel(generator)
discriminator = nn.DataParallel(discriminator)
autoshift = nn.DataParallel(autoshift)
...
# shuffle=False
evaluator = Evaluator_autoshift_save(generator_test, autoshift, eval_loader, ydist,
                      batch_size=batch_size, device=device, config=config)
...
s_scheduler = build_lr_scheduler(s_optimizer, config, last_epoch=it)
...
# )
trainer = Trainer_autoshift(
    generator, discriminator, autoshift, g_optimizer, d_optimizer, s_optimizer,
    gan_type=config['training']['gan_type'],
    reg_type=config['training']['reg_type'],
    reg_param=config['training']['reg_param']
)
...
x = evaluator.create_samples(ytest, batch_size=ntest)
logger.add_imgs(x, 'all', it)
for y_inst in range(sample_nlabels):
    # x = evaluator.create_samples(ztest, y_inst)
    x = evaluator.create_samples(y_inst, batch_size=ntest)
...
while flag:
    ...
    for x_real, y in train_loader:
        ...
        logger.add('learning_rates', 'autoshift', s_lr, it=it)
        ...
        if dist_reg:
            dist_reg_last = logger.get_last('losses', 'dist_reg')
            print('[epoch %0d, it %4d] g_loss = %.4f, dist_reg = %.4f, d_loss = %.4f, reg=%.4f'
                  % (epoch_idx, it, g_loss_last, dist_reg_last, d_loss_last, d_reg_last))
        else:
